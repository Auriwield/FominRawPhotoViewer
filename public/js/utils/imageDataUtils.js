...
var canvasUtils = function () {
    ...
    function drawIntoCanvas(imageData, canvas, scale) {
        ...
        if (scale !== 1) imageData = scaleImageData(imageData, scale);else imageData = new ImageData(imageData.data, imageData.width, imageData.height);

        ctx.putImageData(imageData, 0, 0);
    }

    function makeCanvasSquare(canvas) {
        var subw = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
        var subh = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
    ...
    function scaleImageData(imageData, scale) {
        ...
        for (var y = 0; y < h2; y++) {
            for (var x = 0; x < w2; x++) {
                ...
                if (x1 < 0 || x1 >= w1 || y1 < 0 || y1 >= h1) continue;
                ...
                if (destIndex + 3 >= destLength || destIndex < 0 || sourceIndex + 3 >= srcLength || sourceIndex < 0) continue;
    ...
    function getSquareAtCoords(imageData, x, y) {
        ...
        for (var _i = y, i1 = 0; _i < yEnd; _i++, i1++) {
            for (var j = x, j1 = 0; j < xEnd; j++, j1++) {
                var srcIndex = (_i * imageData.width + j) * 4;
